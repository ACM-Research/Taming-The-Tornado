...
let account_info, addresses_arr, private_keys_dict, private_keys_arr
let noteMap = new Map()
...
async function loadWithdrawalData({ amount, currency, deposit }) {
  try {
    ...
    const decimals = netId > 42 ? 18 : config.deployments[`netId${netId}`][currency].decimals
...
async function ourInit({ rpc, noteNetId, depositAddress, depositKey, currency = 'dai', amount}) {
  let contractJson, erc20ContractJson, erc20tornadoJson, tornadoAddress, tokenAddress
  // TODO do we need this? should it work in browser really?
  if (inBrowser) {
    // Initialize using injected web3 (Metamask)
    ...
    //PRIVATE_KEY = process.env.PRIVATE_KEY
    if (depositKey) {
      const account = web3.eth.accounts.privateKeyToAccount(depositKey)
      web3.eth.accounts.wallet.add(depositKey)
      web3.eth.defaultAccount = account.address
      //senderAccount = (await web3.eth.getAccounts())[0]
      senderAccount = depositAddress
    } else {
      console.log('Warning! depositKey not found. Please provide depositKey in function call')
    }
    erc20ContractJson = require(__dirname + '/../build/contracts/ERC20Mock.json')
    erc20tornadoJson = require(__dirname + '/../build/contracts/ERC20Tornado.json')
  }
...
/**
 * Our modified ETH deposit function
 * @param currency Currency to use
 * @param amount Amount of currency
 */
async function ourDeposit({ currency, amount }) {
  const deposit = createDeposit({ nullifier: rbigint(31), secret: rbigint(31) })
  const note = toHex(deposit.preimage, 62)
  const noteString = `tornado-${currency}-${amount}-${netId}-${note}`
  //console.log("HERE", noteString)
  //console.log(`Your note: ${noteString}`)
  if (currency === 'eth') {
    //console.log("Before")
    //await printETHBalance({ address: tornado._address, name: 'Tornado' })
...
/**
 * Our modified ETH withdrawal function
 * @param deposit Deposit part of note
 * @param currency Currency to use
 * @param amount Amount of currency
 * @param recipient Address to send withdrawal to
 * @param relayerURL SE
 * @param refund IDK
 */
async function ourWithdraw({ deposit, currency, amount, recipient, relayerURL, refund = '0' }) {
  if (currency === 'eth' && refund !== '0') {
    throw new Error('The ETH purchase is supposted to be 0 for ETH withdrawals')
  }
  refund = toWei(refund)
...
/**
 * Simplified ETH deposit function
 * @param depositAddress SE
 */
async function makeDeposit({depositAddress}) {
  return parseNote(await ourDeposit({currency: 'eth', amount: '0.1'}))
}

/**
 * Simplified ETH withdrawal function
 * @param parsedNote SE
 * @param withdrawalAddress SE
 */
async function makeWithdrawal({parsedNote, withdrawalAddress}) {
  await ourWithdraw({ deposit: parsedNote.deposit, currency: 'eth', amount: '0.1', recipient: withdrawalAddress, relayerURL: program.relayer })
}

/**
 * Simplified ETH transaction function
 * @param transactionTime Virtual time value
 * @param type 'deposit' or 'withdrawal' type transaction
 * @param address Address to deposit or withdraw to 
 * @param note Parsed note needed if withdrawing ETH
 */
async function makeTransaction({transactionTime, type, address, note}) {
  if (type ==='deposit') {
    await ourInit({ rpc: program.rpc, depositAddress: address, depositKey: private_keys_dict[address], currency: 'eth', amount: '0.1'})
  }
  let outString = transactionTime+","+type+","+address
  //console.log(`${type} at time ${transactionTime.toFixed(2)} by ${address}`)
  //console.log('\tBefore')
  //await printETHBalance({ address: tornado._address, name: 'Tornado' })
  //await printETHBalance({ address: senderAccount, name: 'Sender account' })
  outString = outString+","+(await getETHBalance({address: tornado._address}))+","+(await getETHBalance({address: senderAccount}))
  if (type === 'deposit') {
    noteMap.set(address, await makeDeposit({depositAddress: address}))
  }
  else if (type === 'withdraw') {
    if (note === '')
      console.error('Withdrawal note should not be empty')
    else
      await makeWithdrawal({parsedNote: note, withdrawalAddress: address})
  }
  else {
    console.error('Transaction type is not recognized, given: '+type)
  }
  //console.log('\tAfter')
  //await printETHBalance({ address: tornado._address, name: 'Tornado' })
  //await printETHBalance({ address: senderAccount, name: 'Sender account' })
...
async function main() {
  if (inBrowser) {
    ...
    window.withdraw = async () => {
      ...
      const recipientList = (await web3.eth.getAccounts());
      const recipient = recipientList[0]
      //IDEAL CIRCUMSTANCE:
      //Specify: number of accounts, gov control %
      //Startup command: ganache-cli -i 1337 -a <num_accounts>
    ...
    program
      .command('simulate <userAccounts> <govControlPercentage>')
      .description('Perform our simulation and outputs results to a .csv file. Uses ganache.')
      .action(async (userAccounts, govControlPercentage) => {
        //console.log('SIMULATION: Starting')
        console.log("time,type,address,beforeTCBalance,beforeUserBalance,afterTCBalance,afterUserBalance")
        const currency = 'eth'
        const amount = '0.1'
        account_info = JSON.parse(fs.readFileSync('./private_keys.json'))
        addresses_arr = Object.keys(account_info["addresses"])
        private_keys_dict = account_info["private_keys"]
        private_keys_arr = Object.values(private_keys_dict)
        userAccounts = parseInt(userAccounts)

        if (2*userAccounts + 1 > addresses_arr.length)
          console.error("private_keys.json must have at least 2*userAccounts+1 addresses")
        
        let govTransacitionAmount = Math.trunc(userAccounts/(1-govControlPercentage)*govControlPercentage)
        //Priority Queue sorts each transaction pair by the time field {time, isWithdrawal, currentAddress, fromAddress}
        var pq = new PriorityQueue({comparator: (aTransaction, bTransaction) => aTransaction.time - bTransaction.time, strategy: PriorityQueue.BinaryHeapStrategy})


        const TIMESPAN = 1440 //Day in minutes
        const impatientMaxOffset = 240 //2hr offset is average according to paper

        //Government Pool Control Setup
        //console.log("SIMULATION: Government Setup Begins")
        for(let i = 0; i < govTransacitionAmount; i++) {
          let t = Math.random() * TIMESPAN
          pq.queue({time: t, isWithdrawal: false, currentAddress: addresses_arr[0], fromAddress: ''})
        }
        //console.log("SIMULATION: Government Setup Ends")

        //Impatient Users Setup
        //console.log("SIMULATION: Impatient Users Setup Begins")
        let t0 = govTransacitionAmount
        //time in in minutes, timespan is a day
        for(let i = 0; i < userAccounts; i++) {
          let t = t0 + Math.random() * TIMESPAN
          let offset = Math.random() * impatientMaxOffset
          let dIndex = i+1
          let wIndex = i+userAccounts+1
          pq.queue({time: t, isWithdrawal: false, currentAddress: addresses_arr[dIndex], fromAddress: ''})
          pq.queue({time: t+offset, isWithdrawal: true, currentAddress: addresses_arr[wIndex], fromAddress: addresses_arr[i+1]})
        }
        //console.log("SIMULATION: Impatient User Setup Ends")

        /*for (let i = 1; i < 5; i++) {
          pq.queue({time: i, isWithdrawal: false, currentAddress: addresses_arr[i-1], fromAddress: ''})
          pq.queue({time: i*5, isWithdrawal: true, currentAddress: addresses_arr[i], fromAddress: addresses_arr[i-1]})
        }*/

        while(pq.length != 0) {
          let {time: transactionTime, isWithdrawal, currentAddress, fromAddress} = pq.dequeue()
          //console.log(transactionTime, isWithdrawal, currentAddress, fromAddress)
          if (!isWithdrawal) {
            await makeTransaction({transactionTime, type: 'deposit', address: currentAddress, note: ''})
          }
          else {
            await makeTransaction({transactionTime, type: 'withdraw', address: currentAddress, note: noteMap.get(fromAddress)})
          }
        }
